...
group_id = config['group_id']  # vkontakte community ID. Note! don't forget “-” in group_id, for example '-98492689'
password = config['mail_password'] # password for your email account
...
popular_comments = sorted(comments_dict.items(), key=lambda x: x[1], reverse=True)[
                   0:21]  # change number of comments here
...
for el in popular_comments:
    ...
    temp_link = ''
    if el[1][-1] != 0:
        temp_link = 'https://vk.com/wall'+str(group_id)+'_' + str(post_id) + '?reply=' + str(comment_id) + '&thread=' + str(
            thread)
    else:
        temp_link = 'https://vk.com/wall'+str(group_id)+'_' + str(post_id) + '?reply=' + str(comment_id)
